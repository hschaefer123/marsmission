...
var n = parseInt((url.indexOf('n=') != -1) ? url.substring(url.indexOf('n=') + 2, ((url.substring(url.indexOf('n=') + 2, url.length)).indexOf(
	'&') != -1) ? url.indexOf('n=') + 2 + (url.substring(url.indexOf('n=') + 2, url.length)).indexOf('&') : url.length) : 512 * 4);
...
lol.init = function() {
	...
	if (window.DeviceOrientationEvent !== undefined) {
		window.addEventListener('deviceorientation', lol.handleOrientation, false);
	} else {
...
lol.viewport = function() {
	...
	$("#" + lol.target).append(el);
	//$( "#__xmlview2--starfield--starfield" ).append(el);
...
lol.resize = function() {
	...
	lol.cvs.style.backgroundColor = 'rgba(0,0,0,0.1)';
...
lol.anim = {
	update: function() {
		...
		var iWarpDepth = 200;
		if (lol.direction === "RIGHT") {
			lol.mouse.p.x = -iWarpDepth;
		} else if (lol.direction === "LEFT") {
			lol.mouse.p.x = iWarpDepth;
		} else if (lol.direction === "WARP") {
			lol.spd = 60;
		} else if (lol.direction === "CENTER") {
			lol.mouse.p.x = 0; //lol.x;
		}
		lol.direction = null;
...
lol.handleOrientation = function(e) {
	if (e.beta !== null && e.gamma !== null) {
		...
		//console.log(lol.portrait, x, e.beta);
		lol.mouse.p.x = x * 4; // multiply 10 for mouse slowdown
