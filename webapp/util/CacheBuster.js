...
CacheBuster.prototype.convertURL = function convertURL(sUrl) {
    ...
    //if (/(xml|js|json|css|properties|png)$/ig.test(extension) && sUrl.search("[?&]ts=") === -1 && sUrl.search("https?:") === -1 && sUrl.search("/resources/") === -1) {
    if (sUrl.search("[?&]ts=") === -1 && sUrl.search("https?:") === -1 && sUrl.search("/resources/") === -1) {
        sUrl = sUrl + "?ts=" + ts;
    }
    return sUrl;
};
